...
{
	int results_counter = 0; //a counter to count the number of occurences
	for(int i=0; i<student_count;i++)
		if(results[i]) //If the result array element at index i was true,
		{
			results_counter++; //increment the counter
			cout <<  results_counter << ". " << narray[i] << " - " << garray[i] <<endl; //print the name and grade of this record
		}
